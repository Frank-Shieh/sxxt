...
<body class="hold-transition skin-blue sidebar-mini">
						...
						<c:if test="${fn:length(errorMsg) gt 0}">
							<div class="bs-example bs-example-standalone">
								<div class="alert alert-danger alert-dismissible fade in"
									...
									<p>暂无相关的人员信息。请先添加实训设备管理员信息</p>
									<p>
										<a class="btn btn-primary" href="/equipmentManager/add"
											style="text-decoration: none;">添加实训设备管理员信息</a>
									</p>
								</div>
							</div>
						</c:if>
						...
						<c:if test="${fn:length(result) gt 0}">
							<div class="panel panel-info">
								<div class="panel-heading">
									<h3 class="panel-title">实训设备管理员信息</h3>
								</div>
											...
											<c:forEach var="result" items="${result}">
												<tr>
													...
													<td><a class="btn btn-info"
														href="/equipmentManager/view?id=${result.id}">查看</a></td>
													<td><a href="/equipmentManager/edit?id=${result.id}"
														class="btn btn-primary">修改</a></td>
													<td><a class="btn btn-danger"
														onclick="delEquipmentManager(${result.id})">删除</a></td>
												</tr>
											</c:forEach>
	...
	function delEquipmentManager(id) {
		var result = confirm("确定删除编号:"+ id +"吗?");
		if (result) {
			console.log(result);
			var url = '/equipmentManager/delete?id='+id;
			$.get(url, function(data) {
				console.log('deleted.');
			 	window.location.href='/equipmentManager/list'; 
			});
		}
	}
</script>
