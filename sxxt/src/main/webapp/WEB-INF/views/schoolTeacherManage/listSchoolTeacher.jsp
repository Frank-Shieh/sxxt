...
<body class="hold-transition skin-blue sidebar-mini">
						...
						<c:if test="${fn:length(result) gt 0}">
							<div class="panel panel-info">
								<div class="panel-heading">
									<h3 class="panel-title">学校教师信息</h3>
								</div>
											...
											<c:forEach var="result" items="${result}">
												<tr>
													...
													<td><a class="btn btn-info"
														href="/schoolTeacher/view?id=${result.id}">查看</a></td>
													<td><a href="/schoolTeacher/edit?id=${result.id}"
														class="btn btn-primary">修改</a></td>
													<td><a class="btn btn-danger"
														onclick="delSchoolTeacher(${result.id})">删除</a></td>
												</tr>
											</c:forEach>
	...
	function delSchoolTeacher(id) {
		var result = confirm("确定删除编号:"+ id +"吗?");
		if (result) {
			console.log(result);
			var url = '/schoolTeacher/delete?id='+id;
			$.get(url, function(data) {
				console.log('deleted.');
			 	window.location.href='/schoolTeacher/list'; 
			});
		}
	}
</script>
