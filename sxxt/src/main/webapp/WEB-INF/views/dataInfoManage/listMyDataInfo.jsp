...
<body class="hold-transition skin-blue sidebar-mini">
								...
								<table id="tp-result-table"
									...
									<tbody>
										<c:forEach var="result" items="${result}">
											<tr>
												<td>${result.dataName}</td>
												<td>${result.downloadTimes}</td>
												<td><a class="btn btn-info"
													href="/dataInfo/view?id=${result.id}">查看</a></td>
												<td><a class="btn btn-primary"
													href="/dataInfo/download/${result.id}" target="download"
													data-toggle="tooltip" title="文件下载">下载</a></td>
												<td><a class="btn btn-danger" 
													onclick="delDataInfo(${result.id},${user.id},${user.role.id})">删除</a></td>
											</tr>
										</c:forEach>
									</tbody>
	...
	function delDataInfo(id,userId,role) {
		var result = confirm("确定删除该资料吗?");
		if (result) {
			console.log(result);
			var url = '/dataInfo/delete/'+id+'/'+userId+'/'+role;
			$.get(url, function(data) {
				console.log('deleted.');
				window.location.href='/dataInfo/listMyself/' + userId + '/' + role;
			});
		}
	}
</script>
