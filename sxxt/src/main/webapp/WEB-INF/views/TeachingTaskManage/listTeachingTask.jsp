...
<body class="hold-transition skin-blue sidebar-mini">
						...
						<c:if test="${fn:length(errorMsg) gt 0}">
							<div class="bs-example bs-example-standalone">
								<div class="alert alert-danger alert-dismissible fade in"
									...
									<p>暂无相关的教学任务信息。请先添加教学任务信息</p>
									<p>
										<a class="btn btn-primary" href="/educationManager/add"
											style="text-decoration: none;">添加实训教学任务</a>
									</p>
								</div>
							</div>
						</c:if>
						...
						<c:if test="${fn:length(result) gt 0}">
							<div class="panel panel-info">
								<div class="panel-heading">
									<h3 class="panel-title">实训教学任务信息</h3>
								</div>
											...
											<c:forEach var="result" items="${result}">
												<tr>
													...
													<td><a class="btn btn-info"
														href="/teachingTask/view?id=${result.id}">查看</a></td>
													<td><a href="/teachingTask/edit?id=${result.id}"
														class="btn btn-primary">修改</a></td>
													<td><a class="btn btn-danger"
														onclick="delTeachingTask(${result.id})">删除</a></td>
												</tr>
											</c:forEach>
	...
	function delTeachingTask(id) {
		var result = confirm("确定删除编号:"+ id +"吗?");
		if (result) {
			console.log(result);
			var url = '/teachingTask/delete?id='+id;
			$.get(url, function(data) {
				console.log('deleted.');
			 	window.location.href='/teachingTask/list'; 
			});
		}
	}
</script>
