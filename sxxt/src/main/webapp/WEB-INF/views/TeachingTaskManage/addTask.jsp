...
<body class="hold-transition skin-blue sidebar-mini">
			...
			<div class="content-wrapper">
				...
				<div class="panel panel-primary">
					<div class="panel-heading">
						<h3 class="panel-title">添加单个实训教师信息</h3>
					</div>
					<div id="breadcrumb">
						<a href="#" title="首页" class="tip-bottom"> <i
							class="icon-home"></i> Home
						</a> <a href="listClass.html">实训教师信息</a> <a href="#" class="current">添加单个实训教师信息</a>
					</div>
					<div class="panel-body">
						<form class="form-horizontal" method="post"
							action="/teachingTask/doAdd" id="registerForm">
								...
								<div class="controls form-group">
									<div class="dropdown btn-group" style="width: 50%">
										<button id="dLabel" class="btn btn-default dropdown-toggle"
											...
											<span id="educationManagerId" value="${educationManagerList.get(0).id}">${educationManagerList.get(0).name}</span>
											<span class="caret"></span>
										</button>
										<input id='educationManager-input' name="educationManager.id"
											value="${educationManagerList.get(0).id}" hidden="true" />
										<ul class="dropdown-menu educationManager-menu"
											aria-labelledby="dLabel" role="menu"
											style="width: 100%; height: 150px; overflow: scroll;">
											<c:forEach var="educationManagerList" items="${educationManagerList}">
	...
	<script>
		...
		$("body").on('click', '.educationManager-menu li', function(e) {
			var $target = $(e.target);
			$target.is('a') && $('#educationManagerId').text($target.text());
			$('#educationManager-input').attr('value', $target.attr("valueid"));
		...
		$('#add-educationManager')
				.on(
						...
						function(e) {
							...
							document.getElementById('add-educationManager-input').value = '';
							$
									.ajax({
										type : "POST",
										dataType : 'json',
										...
										complete : function(msg) {
											...
											if (message.length == 1) {
												...
												var pul = $('.educationManager-menu')
														.parent();
												$('.educationManager-menu').remove();
												...
												ul.appendTo(pul);
												$('.educationManager-menu').parent()
														.show();
												$('.new-educationManager').hide();
